...
### 5.2 实战：学习使用 Parser 转换 ONNX 模型

#### 5.2.1 使用 onnx parser 转换模型的优点与流程

理解

#### 5.2.2 加载 ONNX 模型并比较 PyTorch 结果

理解

#### 5.2.3 构建 ResNet/VGG 模型并推理

理解

#### 5.2.4 打印 TRT 优化前后的网络结构与精度

理解

#### 5.2.5 加载 QAT 模型并分析 TRT 底层优化

理解

#### 5.2.6 使用 onnx surgeon 修改 INNX
...
#### 5.3.1 使用 IPlugin 创建自定义算子的流程

理解

#### 5.3.2 使用 TensorRT python API 创建网络

理解

#### 5.3.3 结合 python 和 C++ 做 plugin 的单元测试
